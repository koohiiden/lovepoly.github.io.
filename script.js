...
window.deleteImage = function(index) {
    if (confirm('Ты точно хочешь удалить эту фотографию? 🥺')) {
        let images = JSON.parse(localStorage.getItem('poly_gallery')) || [];
...
function renderMoments() {
    ...
    moments.forEach((text, index) => {
        ...
        card.innerHTML = `
            <span>${text}</span>
            <button onclick="deleteMoment(${index})" style="
                position: absolute;
                top: 5px;
                right: 5px;
                background: rgba(255, 14, 14, 0.85);
                color: white;
                border: none;
                border-radius: 50%;
                width: 25px;
